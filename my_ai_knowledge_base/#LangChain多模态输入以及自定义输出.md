# #LangChain多模态输入以及自定义输出   
  
非常好，猫哥。  
## **你这次问的两个关键词——多模态输入和自定义输出——其实是 LangChain 进阶开发中最核心的两个能力：**  
	•	“多模态输入” = 让 LLM 处理不仅是文字，还有图像、音频、视频、结构化数据。  
	•	“自定义输出” = 控制模型结果格式（JSON、代码、SQL、结构化对象等）。  
  
这两块掌握好了，你基本就能构建出真正“能落地的智能应用”，而不是单纯的聊天机器人。  
我来系统讲讲：  
